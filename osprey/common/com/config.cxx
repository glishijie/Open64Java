...
static char *rcs_id = "$Source: "
                      "/proj/osprey/CVS/open64/osprey1.0/common/com/"
                      "config.cxx,v $ $Revision: 1.1.1.1 $";
#endif /* _KEEP_RCS_ID */
...
INT32 Heap_Allocation_Threshold =
    -1; /* Allocate objects > this on the heap
...
static OPTION_DESC Options_TENV[] = {
    {OVK_INT32, OV_VISIBLE, FALSE, "align_aggregates", "align_ag", -1, 0, 16,
     &Aggregate_Alignment, NULL,
     "Minimum alignment to use for aggregates (structs/arrays)"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "aligned_copy", NULL, 0, 0, 0,
     &UseAlignedCopyForStructs, NULL},
    {OVK_BOOL, OV_SHY, FALSE, "call_mcount", NULL, 0, 0, 0, &Call_Mcount, NULL},
    {OVK_BOOL, OV_SHY, FALSE, "constant_gp", NULL, 0, 0, 0, &Constant_GP, NULL},
    {OVK_BOOL, OV_SHY, FALSE, "cpic", "cp", 0, 0, 0, &Gen_PIC_Call_Shared, NULL,
     "Generate code for executable programs which may call DSOs"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "fixed_addresses", "fi", 0, 0, 0,
     &PIC_Fixed_Addresses, NULL},
    {OVK_INT32, OV_SHY, FALSE, "Gspace", NULL, DEFAULT_GSPACE, 0, INT32_MAX,
     &Gspace_Available, NULL, "Maximum GP-relative space available"},
    {OVK_UINT32, OV_INTERNAL, FALSE, "ipa_ident", NULL, 0, 0, UINT32_MAX,
     &Ipa_Ident_Number, NULL, "Specify IPA timestamp number"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "kernel", NULL, 0, 0, 0, &Kernel_Code, NULL,
     "Generate code for kernel use"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "large_GOT", "", 0, 0, 0, &Use_Large_GOT,
     NULL, "Assume GOT is larger than 64K bytes"},
    {OVK_NAME, OV_SHY, FALSE, "io_library", NULL, 0, 0, 0, &Library_Name, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "large_stack", NULL, 0, 0, 0,
     &Force_Large_Stack_Model, NULL,
     "Generate code assuming >32KB stack frame"},
#ifdef TARG_X8664
    {OVK_BOOL, OV_INTERNAL, FALSE, "frame_pointer", NULL, 0, 0, 0,
     &Force_Frame_Pointer, &Force_Frame_Pointer_Set,
     "Always allocate and use frame pointer in code generation"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "f2c", NULL, 0, 0, 0, &F2c_Abi, &F2c_Abi_Set,
     "Use g77 linkage convention"},
    {OVK_NAME, OV_INTERNAL, FALSE, "mcmodel", NULL, 0, 0, 0, &Mcmodel_Name,
     NULL, "Set the memory model to us"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "align_double", NULL, 0, 0, 0, &Align_Double,
     &Align_Double_Set,
     "Align double and long double stack variables on a double word boundary"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "simd_imask", NULL, 0, 0, 0, &SIMD_IMask,
     NULL, "Unmask SIMD invalid-operation exception"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "simd_dmask", NULL, 0, 0, 0, &SIMD_DMask,
     NULL, "Unmask SIMD denormalized-operand exception"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "simd_zmask", NULL, 0, 0, 0, &SIMD_ZMask,
     NULL, "Unmask SIMD zero-divide exception"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "simd_omask", NULL, 0, 0, 0, &SIMD_OMask,
     NULL, "Unmask SIMD overflow exception"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "simd_umask", NULL, 0, 0, 0, &SIMD_UMask,
     NULL, "Unmask SIMD underflow exception"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "simd_pmask", NULL, 0, 0, 0, &SIMD_PMask,
     NULL, "Unmask SIMD precision exception"},
#endif
    {OVK_BOOL, OV_VISIBLE, FALSE, "local_names", "", 0, 0, 0, &PIC_Local_Names,
     NULL},
    {OVK_BOOL, OV_SHY, FALSE, "long_eh_offsets", "long_eh", 0, 0, 0,
     &Force_Long_EH_Range_Offsets, NULL},
    // -foptimize-regions implies this option
    {OVK_BOOL, OV_INTERNAL, FALSE, "omit_ue_destroy_frame", "omit_ue", 0, 0, 0,
     &Omit_UE_DESTROY_FRAME, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "non_volatile_GOT", "non_v", 0, 0, 0,
     &Non_Volatile_GOT, NULL, "Assume GOT is non-volatile"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "no_page_offset", "no_p", 0, 0, 0,
     &PIC_No_Page_Offset, NULL, "Don't use GOT page/offset addressing"},
    {OVK_BOOL, OV_SHY, FALSE, "pic2", "pi", 0, 0, 0, &Gen_PIC_Shared, NULL,
     "Generate position-independent code suitable for DSOs"},
    {OVK_BOOL, OV_SHY, FALSE, "pic1", NULL, 0, 0, 0, &Gen_PIC_Call_Shared, NULL,
     "Generate code for executable programs which may call DSOs"},
    {OVK_BOOL, OV_SHY, FALSE, "profile_call", "prof", 0, 0, 0, &Gen_Profile,
     NULL},
    {OVK_NAME, OV_SHY, FALSE, "profile_name", "", 0, 0, 0, &Gen_Profile_Name,
     NULL},
    {OVK_BOOL, OV_VISIBLE, FALSE, "protected_names", "", 0, 0, 0,
     &PIC_Protected_Names, NULL},
    {OVK_INT32, OV_INTERNAL, FALSE, "short_data", "short_d", 0, 0, 4096,
     &Short_Data, &Short_Data_Set,
     "Maximum size of data to allocate GP-relative"},
    {OVK_INT32, OV_INTERNAL, FALSE, "short_literals", "short_l", 0, 0, 1024,
     &Short_Lits, &Short_Lits_Set,
     "Maximum size of literals to allocate GP-relative"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "small_GOT", "sm", 0, 0, 0, &Use_Small_GOT,
     NULL, "Assume GOT is smaller than 64K bytes"},
    {OVK_INT32, OV_INTERNAL, FALSE, "struct_copy_loop_size", "struct_copy_loop",
     -1, 0, 4096, &MinStructCopyLoopSize, NULL},
    {OVK_INT32, OV_INTERNAL, FALSE, "struct_copy_mem_intr_size",
     "struct_copy_mem", -1, 0, 4096, &MinStructCopyMemIntrSize, NULL},
    {OVK_INT32, OV_VISIBLE, FALSE, "X", NULL, 1, 0, 4, &Eager_Level,
     &Eager_Level_Set, "Exception-enable level"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "zeroinit_in_bss", NULL, 0, 0, 0,
     &Zeroinit_in_bss, NULL, "Place zero-initialized data in .bss section"},
    {OVK_BOOL, OV_SHY, FALSE, "strings_not_gprelative", "strings_not_gprel", 0,
     0, 0, &Strings_Not_Gprelative, NULL,
     "Do not put any strings in gp-relative sections"},
    {OVK_NAME, OV_SHY, FALSE, "emit_global_data", "emit_global", 0, 0, 0,
     &Emit_Global_Data, NULL, "only process the global data"},
    {OVK_NAME, OV_SHY, FALSE, "read_global_data", "read_global", 0, 0, 0,
     &Read_Global_Data, NULL, "only read the already-processed global data"},
    {OVK_BOOL, OV_SHY, FALSE, "force_gp_prolog", "force_gp", 0, 0, 0,
     &Force_GP_Prolog, NULL, "force gp_prolog to always be setup"},
    {OVK_LIST, OV_VISIBLE, FALSE, "registers_not_allocatable", NULL, 0, 0, 0,
     &Registers_Not_Allocatable, NULL,
     "list of registers that are reserved and not available for allocation"},

    /***** Options moved elsewhere -- retained for compatibility: *****/
    /* See -DEBUG:div_check */
    {OVK_INT32, OV_INTERNAL, FALSE, "check_div", "check_div", 1, 0, 3,
     &Initial_DEBUG.div_check, &Initial_DEBUG.div_check_set},
    /* See -DEBUG:trap_uninitialized */
    {OVK_BOOL, OV_INTERNAL, FALSE, "trapuv", "", 0, 0, 0,
     &Initial_DEBUG.trap_uv, &Initial_DEBUG.trap_uv_set},
    /* See -DEBUG:trapuv_right_justify */
    {OVK_BOOL, OV_INTERNAL, FALSE, "trapuv_right_justify", "trapuv_right", 0, 0,
     0, &Initial_DEBUG.trap_uv_rjustify, &Initial_DEBUG.trap_uv_rjustify_set},

    /***** Options moved elsewhere -- replaced: *****/
    /* See -DEBUG:varargs_prototypes */
    {OVK_REPLACED, OV_INTERNAL, FALSE, "varargs_prototypes", "varargs_p", 0, 0,
     0, const_cast<char *>("-DEBUG:varargs_prototypes"), NULL},

    /***** Obsolete options: *****/
    {/* OVK_INT32, */
     OVK_OBSOLETE, OV_INTERNAL, FALSE, "align_extern", NULL, 0, 0, 16, NULL,
     NULL, "Assume this alignment for unknown objects"},
    {/* OVK_BOOL, */
     OVK_OBSOLETE, OV_SHY, FALSE, "aligned", NULL, 0, 0, 0, NULL, NULL,
     "Assume unknown objects are properly aligned"},
    {/* OVK_INT32, */
     OVK_OBSOLETE, OV_INTERNAL, FALSE, "misalignment", NULL, 3, 0, 3, NULL,
     NULL},
    {OVK_INT32, OV_INTERNAL, FALSE, "iolist_reuse", "iolist_reuse", 100, 1,
     INT32_MAX, &iolist_reuse_limit, NULL,
...
static OPTION_DESC Options_PHASE[] = {
    {OVK_BOOL, OV_INTERNAL, FALSE, "lno", "l", 0, 0, 0, &Run_lno, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "wopt", "w", 0, 0, 0, &Run_wopt, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "preopt", "p", 0, 0, 0, &Run_preopt, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "cg", "c", 0, 0, 0, &Run_cg, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "clist", NULL, 0, 0, 0, &Run_w2c, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "flist", NULL, 0, 0, 0, &Run_w2f, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "mplist", NULL, 0, 0, 0, &Run_w2fc_early,
     NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "purple", "", 0, 0, 0, &Run_purple, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "ipl", "i", 0, 0, 0, &Run_ipl, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "prompf", NULL, 0, 0, 0, &Run_prompf, NULL},
    {OVK_NAME, OV_INTERNAL, FALSE, "lpath", "", 0, 0, 0, &LNO_Path, NULL},
    {OVK_NAME, OV_INTERNAL, FALSE, "wpath", "", 0, 0, 0, &WOPT_Path, NULL},
    {OVK_NAME, OV_INTERNAL, FALSE, "cpath", "", 0, 0, 0, &CG_Path, NULL},
    {OVK_NAME, OV_INTERNAL, FALSE, "w2cpath", "", 0, 0, 0, &W2C_Path, NULL},
    {OVK_NAME, OV_INTERNAL, FALSE, "w2fpath", "", 0, 0, 0, &W2F_Path, NULL},
    {OVK_NAME, OV_INTERNAL, FALSE, "purpath", "", 0, 0, 0, &Purple_Path, NULL},
    {OVK_NAME, OV_INTERNAL, FALSE, "ipath", "", 0, 0, 0, &Ipl_Path, NULL},
    {OVK_NAME, OV_INTERNAL, FALSE, "tpath", "", 0, 0, 0, &Targ_Path, NULL},
    {OVK_NAME, OV_INTERNAL, FALSE, "prompf_anl_path", "", 0, 0, 0,
     &Prompf_Anl_Path, NULL},
    {OVK_COUNT}};
#elif defined(QIKKI_BE)
static OPTION_DESC Options_PHASE[] = {
    {OVK_NAME, OV_INTERNAL, FALSE, "tpath", "t", 0, 0, 0, &Targ_Path, NULL},
    {OVK_COUNT}};
#endif /* BACK_END */

static OPTION_DESC Options_LANG[] = {
    {OVK_NAME, OV_INTERNAL, FALSE, "", NULL, 0, 0, 0, &Language_Name, NULL,
     "Language being compiled, from front end"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "bool", "", 0, 0, 0, &CXX_Bool_On,
     &CXX_Bool_Set, "C++: enable builtin type 'bool'"},
#ifndef KEY
    // We don't support -LANG:exceptions, keep CXX_Exceptions_On ON by default
    {OVK_BOOL, OV_VISIBLE, FALSE, "exceptions", "", 0, 0, 0, &CXX_Exceptions_On,
     &CXX_Exceptions_Set, "C++: enable exceptions"},
#endif // !KEY
#if 0  // remove it till we have a robust design 
    { OVK_BOOL, OV_SHY,		FALSE, "alias_const",		"",
      0, 0, 0,  &CXX_Alias_Const,       &CXX_Alias_Const_Set },
#endif
    {OVK_BOOL, OV_VISIBLE, FALSE, "recursive", "", 0, 0, 0, &LANG_Recursive,
     &LANG_Recursive_Set, "FORTRAN: program contains recursion"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "wchar_t", "", 0, 0, 0, &CXX_Wchar_On,
     &CXX_Wchar_Set, "C++: enable builtin type 'wchar_t'"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "namespaces", "namespace", 0, 0, 0,
     &CXX_Namespaces_On, &CXX_Namespaces_Set, "C++: enable namespaces"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "ansi-for-init-scope", "", 0, 0, 0,
     &CXX_Ansi_For_Init_Scope_On, &CXX_Ansi_For_Init_Scope_Set},
    {OVK_BOOL, OV_VISIBLE, FALSE, "std", "", 0, 0, 0,
     &CXX_Standard_C_Plus_Plus_On, &CXX_Standard_C_Plus_Plus_Set},
    {OVK_BOOL, OV_SHY, FALSE, "restrict", "", 0, 0, 0, &C_Restrict_On,
     &C_Restrict_Set},
    {OVK_NAME, OV_VISIBLE, FALSE, "autorestrict", NULL, 0, 0, 0,
     &C_Auto_Restrict, &C_Auto_Restrict_Set,
     "Automatically set the \"restrict\" qualifier on some or all pointers"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "scalar_formal_ref", "", 0, 0, 0,
     &Scalar_Formal_Ref, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "non_scalar_formal_ref", "", 0, 0, 0,
     &Non_Scalar_Formal_Ref, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "short_circuit_conditionals", "", 0, 0, 0,
     &FTN_Short_Circuit_On, &FTN_Short_Circuit_Set},
    {OVK_BOOL, OV_VISIBLE, FALSE, "vla", "", 0, 0, 0, &C_VLA_On, &C_VLA_Set,
     "C/C++: enable variable length arrays"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "explicit", "", 0, 0, 0, &CXX_Explicit_On,
     &CXX_Explicit_Set, "C++: enable explicit keyword"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "typename", "", 0, 0, 0, &CXX_Typename_On,
     &CXX_Typename_Set, "C++: enable typename keyword"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "mutable", "", 0, 0, 0, &CXX_Mutable_On,
     &CXX_Mutable_Set, "C++: enable mutable keyword"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "macro_expand_pragmas", "", 0, 0, 0,
     &Macro_Expand_Pragmas_On, &Macro_Expand_Pragmas_Set,
     "C/C++: enable macro expansion in pragmas"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "packed", "", 0, 0, 0, &CXX_Packed_On,
     &CXX_Packed_Set, "C++: enable pragma pack"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "symtab_verify", "", 0, 0, 0,
     &LANG_Symtab_Verify_On, &LANG_Symtab_Verify_Set},
    {OVK_BOOL, OV_VISIBLE, FALSE, "pch", NULL, 0, 0, 0, &LANG_Pch,
     &LANG_Pch_Set, "Create a precompiled header for this compilation unit"},
    {OVK_NAME, OV_VISIBLE, FALSE, "create_pch", NULL, 0, 0, 0, &LANG_Create_Pch,
     &LANG_Create_Pch_Set,
     "Create a precompiled header file named by this option"},
    {OVK_NAME, OV_VISIBLE, FALSE, "use_pch", NULL, 0, 0, 0, &LANG_Use_Pch,
     &LANG_Use_Pch_Set, "Use the precompiled header file named by this option"},
    {OVK_NAME, OV_VISIBLE, FALSE, "pch_dir", NULL, 0, 0, 0, &LANG_Pchdir,
     &LANG_Pchdir_Set, "Create/Use from the directory named by this option"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "ansi_setjmp", "", 0, 0, 0,
     &LANG_Ansi_Setjmp_On, &LANG_Ansi_Setjmp_Set,
    ...
    {OVK_INT32, OV_VISIBLE, TRUE, "heap_allocation_threshold", "heap_a", -1, -1,
     INT32_MAX, &Heap_Allocation_Threshold, NULL,
     "Size threshold for switching from stack to heap allocation"},
    {OVK_NAME, OV_VISIBLE, FALSE, "cxx_dialect", NULL, 0, 0, 0,
     &LANG_cxx_dialect, &LANG_cxx_dialect_Set},
    {OVK_BOOL, OV_VISIBLE, FALSE, "ignore_carriage_return", "", 0, 0, 0,
     &LANG_Ignore_Carriage_Return_On, &LANG_Ignore_Carriage_Return_Set,
     "C/C++: ignore carriage returns in source"},
#ifdef KEY
    {OVK_BOOL, OV_VISIBLE, FALSE, "rw_const", "", 0, 0, 0,
     &LANG_Read_Write_Const, &LANG_Read_Write_Const,
     "FORTRAN: store the constant in a temporary location and pass the address "
     "of the temporary location"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "formal_deref_unsafe", "", 0, 0, 0,
     &LANG_Formal_Deref_Unsafe, &LANG_Formal_Deref_Unsafe,
     "FORTRAN: the dereference of a formal parameter is unsafe, so do not "
     "speculate such dereferences"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "copyinout", "", 0, 0, 0, &LANG_Copy_Inout,
     &LANG_Copy_Inout_Set,
     "FORTRAN: In a loop, pass noncontiguous array by copy-in-out to improve "
     "locality"},
    {OVK_BOOL, OV_VISIBLE, FALSE, "ignore_target_attribute", "", 0, 0, 0,
     &LANG_Ignore_Target_Attribute, &LANG_Ignore_Target_Attribute_Set,
     "FORTRAN: Assume no actual parameter has the 'target' attribute"},
    {OVK_UINT32, OV_VISIBLE, FALSE, "copyiolevel", "", 0, 0, INT32_MAX,
     &LANG_Copy_Inout_Level, NULL, "FORTRAN: Relax constraints on 'copyinout'"},
    {OVK_BOOL, OV_VISIBLE, TRUE, "IEEE_save", "", 0, 0, 0, &LANG_IEEE_Save,
     &LANG_IEEE_Save_Set,
     "FORTRAN: Save/restore FPU state in procedure prolog/epilog as F2003 "
     "requires"},
#endif
#ifdef KEY
    /* Bug 3405 */
    {OVK_BOOL, OV_VISIBLE, TRUE, "IEEE_minus_zero", "", 0, 0, 0,
     &LANG_IEEE_Minus_Zero_On, &LANG_IEEE_Minus_Zero_Set,
     "FORTRAN: SIGN intrinsic considers -0.0 input to be negative"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "math_errno", "", 0, 0, 0, &LANG_Math_Errno,
     &LANG_Math_Errno_Set, "C/C++: Set errno after calling math functions"},
#endif /* KEY */
...
static OPTION_DESC Options_INTERNAL[] = {
    ...
    {OVK_BOOL, OV_INTERNAL, FALSE, "comma_rcomma", "", 0, 0, 0,
     &WHIRL_Comma_Rcomma_On, &WHIRL_Comma_Rcomma_Set},
    {OVK_BOOL, OV_INTERNAL, FALSE, "merge_types", "", 0, 0, 0,
     &WHIRL_Merge_Types_On, &WHIRL_Merge_Types_Set},
    {OVK_BOOL, OV_INTERNAL, FALSE, "mtype_a", NULL, 0, 0, 0, &WHIRL_Mtype_A_On,
     NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "mtype_b", NULL, 0, 0, 0, &WHIRL_Mtype_B_On,
     NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "mtype_bs", NULL, 0, 0, 0,
     &WHIRL_Mtype_BS_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "return_val", NULL, 0, 0, 0,
     &WHIRL_Return_Val_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "flatten_field", NULL, 0, 0, 0,
     &WHIRL_Flatten_Field_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "mldid_mstid", NULL, 0, 0, 0,
     &WHIRL_Mldid_Mstid_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "vfcall", NULL, 0, 0, 0, &WHIRL_Vfcall_On,
     NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "addr_passed", NULL, 0, 0, 0,
     &WHIRL_Addr_Passed_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "addr_saved_for_passed", NULL, 0, 0, 0,
     &WHIRL_Addr_Saved_For_Passed_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "addr_saved", NULL, 0, 0, 0,
     &WHIRL_Addr_Saved_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "keep_cvt", NULL, 0, 0, 0,
     &WHIRL_Keep_Cvt_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "return_info", NULL, 0, 0, 0,
     &WHIRL_Return_Info_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "global_pragmas", NULL, 0, 0, 0,
     &Global_Pragmas_In_Dummy_PU_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "malloc_free", NULL, 0, 0, 0,
     &Malloc_Free_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "alloca_dealloca", NULL, 0, 0, 0,
     &Alloca_Dealloca_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "barrier_lvalues", NULL, 0, 0, 0,
     &Barrier_Lvalues_On, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "mask_shift_counts", NULL, 0, 0, 0,
     &ARCH_mask_shift_counts, NULL},
    {OVK_BOOL, OV_INTERNAL, FALSE, "generate_nor", NULL, 0, 0, 0,
     &ARCH_generate_nor, NULL},
#ifdef KEY
    {OVK_NAME, OV_INTERNAL, FALSE, "old_ld_lib_path", "", 0, 0, 0,
     &IPA_old_ld_library_path, NULL},
    {OVK_NAME, OV_INTERNAL, FALSE, "lang", "", 0, 0, 0, &IPA_lang, NULL},
#endif
...
OPTION_GROUP Common_Option_Groups[] = {
    {"DEBUG", ':', '=', Options_DEBUG, NULL, "Options to assist debugging"},
...
static void Configure_Platform(char *platform_name) {
   ...
   * don't re-default it:
   */
  if (Platform != IP0 && (platform_name == NULL || *platform_name == 0)) {
    return;
  }
...
void Configure(void) {
  ...
  if (Emit_Global_Data && Read_Global_Data) {
    ...
    Global_File_Name = Emit_Global_Data;
  else if (Read_Global_Data)
    Global_File_Name = Read_Global_Data;
...
/**  NOTE: filename CAN BE NULL */
{
  ...
  if (Mcmodel_Name == NULL) {
    ...
    FmtAssert(
        Is_Target_64bit(),
        ("code model `%s' not supported in the 32-bit mode", Mcmodel_Name));
  ...
  if (!IEEE_Arith_Set && OPT_Funsafe_Math_Optimizations_Set) {
    ...
    //                                =OFF => IEEE_a == 1
    IEEE_LEVEL accuracy =
        OPT_Funsafe_Math_Optimizations ? IEEE_ANY : IEEE_ACCURATE;
    if (accuracy > IEEE_Arithmetic)
      IEEE_Arithmetic = accuracy;
  }
#endif
  ...
  Enable_GDSE = ((Opt_Level > 1) && (!Get_Trace(TP_GLOBOPT, 4096)));
  /* The lowerer will do a simple treeheight reduction for
   * binary commutative ops
   */

#if !defined(TARG_X8664) && !defined(TARG_IA32)
  /* For the x86 architecture, OPT_Lower_Treeheight should be set by user
     directly with option -OPT:treeheight option; otherwise, results will be
     inaccurate. (mgrid is an example.)
  */
...
static void Print_Skiplist(FILE *tf, SKIPLIST *skip, char *lab) {
  INT32 i;
...
SKIPLIST *Build_Skiplist(OPTION_LIST *olist) {
  ...
  /* Fill the skiplist: */
  for (count = 0, ol = olist; ol != NULL; ++count, ol = OLIST_next(ol)) {
...
BOOL Process_Trace_Option(char *option) {
  ...
  while (cp && *cp == 't') {
    ...
    switch (*(cp - 1)) {
    case 'a':
      Set_Trace(TKIND_ALLOC, Get_Trace_Phase_Number(&cp, option));
      break;

    case 'b':
      Set_Trace(TKIND_BB, Get_Numeric_Flag(&cp, 0, 32767, 0, option));
      break;

    case 'c':
      Set_Trace(TKIND_CTRL, Get_Numeric_Flag(&cp, 0, 32767, 0, option));
      break;

    case 'd':
      Set_Trace(TKIND_DEBUG, Get_Numeric_Flag(&cp, 1, 32767, 0, option));
      break;

    /* ex: -Wb,-tf0 for function 0 in the file */
    case 'f':
      if (isdigit(*cp)) {
        Set_Trace_Pu_Number(Get_Numeric_Flag(&cp, 0, 0x7fffffff, 0, option));
      } else {
        Set_Trace_Pu(cp);
    ...
    case 'i':
      Set_Trace(TKIND_INFO, Get_Numeric_Flag(&cp, 1, 32767, 0, option));
      break;

    case 'n':
      Set_Trace(TKIND_TN, Get_Trace_Phase_Number(&cp, option));
      break;
    ...
    case 'r':
      Set_Trace(TKIND_IR, Get_Trace_Phase_Number(&cp, option));
      break;

    case 's':
      Set_Trace(TKIND_SYMTAB, Get_Trace_Phase_Number(&cp, option));
    ...
    case 't':
      ...
      cp++; /* skip separator */
      Set_Trace(phase, Get_Numeric_Flag(&cp, 1, 0xffffffff, 0, option));
      break;
...
void List_Compile_Options(FILE *f, char *pfx, BOOL internal, BOOL full,
                          BOOL update) {
  char *bar = SBar + 12; /* Shorten it a bit */

  fprintf(f, "\n%s%s%s Compiling %s (%s)\n%s%s", pfx, bar, pfx, Src_File_Name,
          Irb_File_Name, pfx, bar);
  fprintf(f, "\n%s%s%s Options:\n%s%s", pfx, bar, pfx, pfx, bar);

  fprintf(f, "%s  Target:%s, ISA:%s, Endian:%s, Pointer Size:%d\n", pfx,
          Targ_Name(Target), Isa_Name(Target_ISA),
