...
void Cleanup_Files(BOOL report, BOOL delete_dotofile) {
  ...
  if (IR_File != NULL && fclose(IR_File)) {
    if (report)
      ErrMsg(EC_IR_Close, IR_File_Name, errno);
  }
  IR_File = NULL;
  if (Delete_IR_File && unlink(IR_File_Name)) {
    if (report)
      ErrMsg(EC_IR_Delete, IR_File_Name, errno);
  }
...
void Warning(const char *msg) {
  fprintf(stderr, "%s: In function %s:\n", Orig_Src_File_Name,
          ST_name(Scope_tab[CURRENT_SYMTAB].st));
...
void WGEN_Init(INT argc, char **argv, char **envp) {
...
void WGEN_File_Init(INT argc, char **argv) {
...
void WGEN_Finish() { WGEN_Stmt_Stack_Free(); }

void WGEN_Check_Errors(int *error_count, int *warning_count,
                       BOOL *need_inliner) {
...
char *WGEN_Stmt_Kind_Name[wgen_stmk_last + 1] = {"'unknown'",
                                                 "'function entry'",
                                                 "'function pragma'",
                                                 "'function body'",
                                                 "'region pragmas'",
...
static void WGEN_Stmt_Stack_Init(void) {
  ...
  wn_stmt_stack = (WN_STMT *)malloc(sizeof(WN_STMT) * wn_stmt_stack_size);
...
void WGEN_Stmt_Push(WN *wn, WGEN_STMT_KIND kind, SRCPOS srcpos) {
  INT new_stack_size;
...
bool Check_For_Call_Region(void) {
  if (key_exceptions) {
...
void WGEN_Stmt_Append(WN *wn, SRCPOS srcpos) {
  WN *body;
  WN *last;
...
WN *WGEN_Stmt_Pop(WGEN_STMT_KIND kind) {
  WN *wn;
  FmtAssert(wn_stmt_sp >= wn_stmt_stack,
  ...
  if (key_exceptions && wn_stmt_sp->kind != kind) {
    ...
    } else { // If we got an opportunity but did not close the region earlier in
  ...
  FmtAssert(wn_stmt_sp->kind == kind,
            ("mismatch in statements: expected %s, got %s\n",
             WGEN_Stmt_Kind_Name[kind], WGEN_Stmt_Kind_Name[wn_stmt_sp->kind]));
...
// this function).
void WGEN_Stmt_Prepend_Last(WN *wn, SRCPOS srcpos) {
  WN *body;
  WN *last;
...
WN *WGEN_Find_Stmt_In_Stack(WGEN_STMT_KIND kind) {
